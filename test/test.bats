...
@test "fetch_secret retrieves secrets successfully" {
  ...
  [ "$status" -eq 0 ]
  [ "$output" = "Batch retrieval of secrets succeeded." ]
  [ "$(cat $BASH_ENV)" = "export SECRET1=value1\nexport SECRET2=value2" ]
}

# Mock the curl command to simulate a failure in batch retrieval
@test "fetch_secret falls back to single secret fetch on batch retrieval failure" {
  run() {
    fetch_secret() {
  ...
  run fetch_secret
  [ "$status" -eq 1 ]
  [ "$output" = "Secret(s) are empty or not found :: secret2, " ]
}
